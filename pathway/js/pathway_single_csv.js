...
var graph_data_json;
let fullCsvData = []; // To store the entire parsed CSV data
...
var make_graph = async (json_path_or_data) => {
    ...
    if (!json_path_or_data) {
        ...
        showNoDataMessage('No data to display. Please check your selections.');
        return;
    }
...
function showNoDataMessage(text = '📁 Upload a single merged CSV file to begin.') {
    myChart.clear();
    myChart.setOption({
        graphic: {
            elements: [{
...
function nodeLinkMergePathways(allTables, pairwiseCompares) {
    const nameColor = Object.fromEntries(pairwiseCompares.map((k, i) => [k, colors[i % colors.length]]));
...
async function handleFileData(data) {
    const separateSets = nodeLinkSeparateSets(data.allTables, data.pairwiseCompares);
    const mergePathway = nodeLinkMergePathways(data.allTables, data.pairwiseCompares);
            ...
            mixCategory.itemStyle = {
                color: {
                    ...
                    colorStops: [
                        ...
                        { offset: 0.255, color: "#5470c6" },
                        { offset: 0.255, color: "#91cc75" },
                        { offset: 0.51, color: "#91cc75" },
                        { offset: 0.51, color: "#fac858" },
...
document.getElementById('csvInput').addEventListener('change', async function() {
    ...
    const dataSelectionControls = document.getElementById('data-selection-controls');
    
    if (file) {
        preview.innerHTML = `File selected: ${file.name}`;
        showLoadingOverlay('Parsing CSV file...');
        try {
            fullCsvData = await parseCSVFile(file);
            
            const groups = [...new Set(fullCsvData.map(row => row.group))];
            ...
            function updateRegionSelect() {
                ...
                const regionsForGroup = [...new Set(fullCsvData
                    .filter(row => row.group === selectedGroup)
                    .map(row => row.region))];
                
                regionSelect.innerHTML = regionsForGroup.map(r => `<option value="${r}">${r}</option>`).join('');
            }

            groupSelect.addEventListener('change', updateRegionSelect);
            updateRegionSelect(); // Initial population
            
            dataSelectionControls.classList.remove('hidden');
            hideLoadingOverlay();
            alert('CSV file parsed. Please select a group and region, then click "Generate Graph".');
        ...
        preview.innerHTML = '';
        dataSelectionControls.classList.add('hidden');
    }
});
...
document.getElementById('generate-graph').addEventListener('click', async () => {
    ...
    if (!selectedGroup || !selectedRegion) {
        alert('Please select both a group and a region.');
        return;
    }

    try {
        showLoadingOverlay('Generating graph...');
        myChart.showLoading('Processing data...');

        const filteredData = fullCsvData.filter(row => row.group === selectedGroup && row.region === selectedRegion);
        
        if (filteredData.length === 0) {
            myChart.hideLoading();
            hideLoadingOverlay();
            showNoDataMessage(`No data found for Group: ${selectedGroup} and Region: ${selectedRegion}.`);
            alert('No data found for the selected combination.');
            return;
        }

        const comparisons = [...new Set(filteredData.map(row => row.comparison))];
        const tables = comparisons.map(comp => filteredData.filter(row => row.comparison === comp));

        const dataForGraph = {
            allTables: tables,
            pairwiseCompares: comparisons
        };

        const result = await handleFileData(dataForGraph);
        ...
        myChart.hideLoading();
        hideLoadingOverlay();
        document.getElementById('nodeMerge_switch').checked = true;
        make_graph(result.mergePathway);
        
        alert(`Graph generated for Group: ${selectedGroup}, Region: ${selectedRegion}`);
    } catch (error) {
        myChart.hideLoading();
        hideLoadingOverlay();
...
function showLoadingOverlay(text = "Processing file...") {
